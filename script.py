...
#Windows Format
library_dir = r'G:\3D Printer Models\Project'
library_dir = re.sub(r'\\', r'/',library_dir)
#library_dir = r'C:\Your_STL_Library_Folder\
#Linux Format
...
image_creation_flag = 'Y'
html_creation_flag = 'Y'
width = 500
height = 500
anim_frames = 16
anim_seconds = 3000
...
#check library directory to make sure folder is actually available
if not (os.path.isdir(library_dir)):
    print("Library folder does not exist. Please check directory")
    image_creation_flag = 'N'
    html_creation_flag = 'N'
...
file_paths = list(glob.iglob(library_dir + "/" + r'**\*.stl', recursive=True))
file_paths.sort
...
if image_creation_flag == 'Y':
    ...
    for i, fp in enumerate(file_paths):
        print("")
        print("Beginning Object Processing: ", fp)
        print("")
        # Removes .stl file extension from filepath
        fp_no_ext = re.sub(r'\.stl', r'', fp) 
        if (0 <= i <= 10): #set this to a low value for testing
            #load model
            bpy.ops.import_mesh.stl(filepath=fp,axis_forward=axis_forward_parm,axis_up=axis_up_parm)
            ...
            for index in range(anim_frames):
                ...
                scene.camera.rotation_euler = (radians(75), 0.0, radians(angle))
        ...
        print("")        
        print("Complete Processing Object: ", fp)
        print("")
                    
    print("")
    print("Completed Library Image Creation")
    print("")

if html_creation_flag == 'Y':
    ########################################
    #        
    # HTML Page Creation
    #
    ...
    print("")
    print("Beginning Library Catalogue Creation")
    print("")
    ...
    css_template= """
            ...
            div.img{0}:hover {{
                background-image: url("file:///{1}_anim.gif");
    ...
    print("")
    print("Completed Library Catalogue Creation")
    print("")
